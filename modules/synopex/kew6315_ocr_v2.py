...
from modules.synopex.kew6315_layout_v2 import (
    KEW6315_REF_HEIGHT,
    KEW6315_REF_WIDTH,
    SCREEN_BY_INDEX,
)
...
def _load_templates(digits_dir: str = str(DEFAULT_DIGITS_DIR)) -> dict[str, list[DigitTemplate]]:
    ...
    for color in result:
        for char in SUPPORTED_CHARS:
            ...
            contrast = _contrast_from_background(arr, BACKGROUND_BY_KEY[color])
...
def _match_template(
    ...
    if left < 0 or right >= field_contrast.shape[1]:
        return None
...
def read_kew6315_screen_fields(
    ...
    for overlay in screen["overlays"]:
        ...
        if selected and field_id not in selected:
            continue
        result[field_id] = _read_field(_crop_field(screen_arr, overlay), overlay.get("bg", "w"), digits_path)
